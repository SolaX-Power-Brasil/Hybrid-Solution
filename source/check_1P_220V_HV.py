...
def singlePhaseSolution(file_path: str, \
    ...
    #Dados para teste
    if not load_powerW:
        load_powerW = int(input("Potência de regime da carga [W]:"))
    if not load_peak_powerW:
        load_peak_powerW = int(input("Potência de pico da carga [W]: "))
    if not load_energyWh:
        load_energyWh = int(input("Energia necessária [Wh]:"))
    #if
